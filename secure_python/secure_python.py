...
def ems(dateiname: str):

#    Kodiert und verschlüsselt eine Python-Datei:
#    - Erstellt ein Mapping (.json)
#    - Verschlüsselt es als .enc
#    - Entfernt die .json-Datei
#    - Originaldatei bleibt erhalten
    
#    Parameter:
#        dateiname (str): Name der Python-Datei (z. B. "main.py")


    # Pfade vorbereiten
    json_path = sp() + dateiname.removesuffix(".py") + ".json"
    enc_path = json_path.removesuffix(".json") + ".enc"

    # Mapping erzeugen und als JSON speichern
    mapping = crm()               # Erzeuge zufälliges Mapping
    sm(mapping, dateiname)        # Speichere Mapping als .json-Datei

    # JSON-Mapping-Datei einlesen
    with open(json_path, "rb") as file:
        json_data = file.read()

    # AES-GCM Verschlüsselung vorbereiten
    salt = get_random_bytes(16)
    key = PBKDF2(p, salt, dkLen=32, count=100_000)
    cipher = AES.new(key, AES.MODE_GCM)
    ciphertext, tag = cipher.encrypt_and_digest(json_data)

    # Verschlüsselte Datei schreiben (.enc)
    with open(enc_path, "wb") as out_file:
        out_file.write(salt + cipher.nonce + tag + ciphertext)

    # Ursprüngliche Mapping-Datei löschen
    os.remove(json_path)

    # Python-Datei mit Mapping kodieren und als .lpyip speichern
    ef(dateiname, mapping)



def emd(dateiname: str):
#    Kodiert und verschlüsselt eine Python-Datei:
#    - Erstellt ein Mapping (.json)
#    - Verschlüsselt es als .enc
#    - Entfernt die .json- und .py-Datei
...
#        dateiname (str): Name der Python-Datei (z. B. "main.py")
    ...
    # Mapping erzeugen und speichern
    mapping = crm()              # Custom random mapping
    sm(mapping, dateiname)       # Save mapping to .json

    # JSON-Datei einlesen (Mapping-Daten)
    with open(json_path, "rb") as file:
        json_data = file.read()

    # Verschlüsselung vorbereiten (AES-GCM)
    salt = get_random_bytes(16)
    key = PBKDF2(p, salt, dkLen=32, count=100_000)
    cipher = AES.new(key, AES.MODE_GCM)
    ciphertext, tag = cipher.encrypt_and_digest(json_data)
    ...
    os.remove(json_path)         # Entferne Mapping-Datei
    ef(dateiname, mapping)       # Encode file mit Mapping (.lpyip)
    os.remove(dateiname)         # Entferne Original-Python-Datei



def dme(dateiname: str, namespace=None):
#    Entschlüsselt und dekodiert eine .enc-Datei, führt den enthaltenen Python-Code aus.

#    Parameter:
#        dateiname (str): Name der zu ladenden Python-Datei (z. B. "beispiel.py")
#        namespace (dict, optional): Ziel-Namespace für die Ausführung (Standard: globals())
    ...
    try:
        # Pfade vorbereiten
        enc_path = sp() + dateiname.removesuffix(".py") + ".enc"
        code_path = sp() + "data/" + dateiname.removesuffix(".py") + ".lpyip"

        # Verschlüsselte Mapping-Datei laden
        with open(enc_path, "rb") as enc_file:
            data = enc_file.read()

        # Daten aufteilen: Salt, Nonce, Tag, Ciphertext
        salt = data[:16]
        nonce = data[16:32]
        tag = data[32:48]
        ciphertext = data[48:]
        ...
        # Mapping decodieren und umkehren
        raw_mapping = json.loads(plaintext.decode("utf-8"))
        mapping = {k2z(k): v for k, v in raw_mapping.items()}
        reverse_mapping = {v: k for k, v in mapping.items()}

        # Kodierten Quellcode lesen und dekodieren
        ...
        print(f"Fehler beim Entschlüsseln oder Ausführen von '{dateiname}': {e}")




def dms(dateiname: str, file_path="" , marked=False ,cleanup=False, namespace=None,):
    #Entschlüsselt und dekodiert eine .enc-Datei und schreibt das Ergebnis als .py-Datei.
    ...
    # Mapping aus der JSON-Struktur laden
    raw_mapping = json.loads(plaintext.decode("utf-8"))

    # Zeichen zurück in Python-Code übersetzen
    mapping = {k2z(k): v for k, v in raw_mapping.items()}
    reverse_mapping = {v: k for k, v in mapping.items()}
    if DEBUG == True:
        print(f"mapping is \n {mapping}")
    ...
    # Zeichen dekodieren
    decoded_code = ''.join(reverse_mapping.get(c, '?') for c in coded_text)
    if DEBUG == True:
    ...
    if marked == True:
        output_file = file_path + dateiname.removesuffix(".py") + "__encoded__.py"
        if DEBUG == True:
            print(f" the the path of the file is: {output_file}")

    elif marked == False:
        output_file = file_path + dateiname
        if DEBUG == True:
            print(f" the the path of the file is: {output_file}")
    ...
    if cleanup == True:
        out_file = sp() + "\\data\\" + dateiname.removesuffix(".py") + ".lpyip"
